...
int main(){
    ...
    x=x-4.0; // ลบค่า 4.0 ออกจาก x
    x=6.5*x; // คูณ x ด้วย 6.5   
    x=fmod(x,(y+z*a)); // หารเอาเศษของ x ด้วย (y + z * a)
    x=x/(2.0*x); // หาร x ด้วย (2.0 * x)
    total=total+(price*quantity-discount); // คำนวณ total โดยนำ price คูณกับ quantity แล้วลบด้วย discount 
    x=x*(1+rate/100.0); // คูณ x ด้วย (1 + rate / 100.0)
    score=score-(mistake*penalty); // ลบค่า (mistake * penalty) ออกจาก score
